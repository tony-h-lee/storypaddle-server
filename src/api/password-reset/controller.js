...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your bladeseeker-server account.<br>
        ...
        &mdash; bladeseeker-server Team
      `
      return sendMail({ toEmail: email, subject: 'bladeseeker-server - Password Reset', content })
    })
